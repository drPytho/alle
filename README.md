...
### Testing with PostgreSQL (Manual Setup)
...
2. Run the bridge:
```bash
POSTGRES_URL="postgresql://postgres:password@localhost/postgres" cargo run
```

3. In another terminal, send a notification:
```bash
# Using alle publish command
cargo run -- -p "postgresql://postgres:password@localhost/postgres" \
  publish notifications "test message"

# Or using psql
psql postgresql://postgres:password@localhost/postgres -c "NOTIFY notifications, 'test message'"
```
...
# Server responds: {"type":"subscribed","channel":"orders"}

# Subscribe to another channel
{"type": "subscribe", "channel": "alerts"}
# Server responds: {"type":"subscribed","channel":"alerts"}

# Send a notification to Postgres
{"type": "notify", "channel": "orders", "payload": "New order!"}

# In another terminal, trigger a Postgres notification:
# psql postgresql://localhost/postgres -c "NOTIFY orders, 'Order #123'"

# You'll receive: {"type":"notification","channel":"orders","payload":"Order #123"}

# Unsubscribe from a channel
{"type": "unsubscribe", "channel": "alerts"}
# Server responds: {"type":"unsubscribed","channel":"alerts"}
```

### Testing with JavaScript

A complete HTML client example is provided in `examples/client_example.html`. Open it in your browser to:
- Subscribe/unsubscribe to channels interactively
- Send notifications to Postgres
- See real-time messages in a log
